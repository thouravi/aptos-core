...
impl BatchAggregator {
    ...
    pub(crate) fn end_batch(
        ...
        let mut append_res = self.append_transactions(batch_id, fragment_id, transactions);
        if append_res == Err(BatchAggregatorError::MissedFragment) && self.batch_state.is_some() {
            // Some fragment was missed for lower batches, but the current batch was a single
            // fragment and was appended successfully (since the state exists).
            append_res = Ok(());
        }
