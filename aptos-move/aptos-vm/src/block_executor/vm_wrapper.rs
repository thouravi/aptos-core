...
use crate::{
    ...
    aptos_vm::AptosVM,
    block_executor::{storage_wrapper::AptosVersionedView, AptosTransactionOutput},
    data_cache::StorageAdapter,
    logging::AdapterLogSchema,
};
use aptos_aggregator::{delta_change_set::DeltaChangeSet, transaction::TransactionOutputExt};
use aptos_block_executor::task::{DataView, ExecutionStatus, ExecutorTask};
...
impl<'a, S: 'a + StateView> ExecutorTask for AptosExecutorTask<'a, S> {
    ...
    fn execute_transaction(
        &self,
        view: &dyn DataView<T = PreprocessedTransaction>,
        txn: &PreprocessedTransaction,
        txn_idx: usize,
        materialize_deltas: bool,
    ) -> ExecutionStatus<AptosTransactionOutput, VMStatus> {
        let log_context = AdapterLogSchema::new(self.base_view.id(), txn_idx);
        let view = AptosVersionedView::new_view(self.base_view, view);

        match self.vm.execute_single_transaction(txn, &view, &log_context) {
