...
pub enum EntryFunctionCall {
    ...
    /// @param execution_hash Required. This is the hash of the resolution script. When the proposal is resolved,
    /// only the exact script with matching hash can be successfully executed.
    AptosGovernanceCreateSingleStepOrMultiStepProposal {
        stake_pool: AccountAddress,
        execution_hash: Vec<u8>,
        metadata_location: Vec<u8>,
        metadata_hash: Vec<u8>,
...
impl EntryFunctionCall {
    ...
    pub fn encode(self) -> TransactionPayload {
        ...
        match self {
            ...
            } => aptos_governance_create_single_step_or_multi_step_proposal(
                stake_pool,
                execution_hash,
                metadata_location,
                metadata_hash,
...
/// @param execution_hash Required. This is the hash of the resolution script. When the proposal is resolved,
/// only the exact script with matching hash can be successfully executed.
pub fn aptos_governance_create_single_step_or_multi_step_proposal(
    stake_pool: AccountAddress,
    execution_hash: Vec<u8>,
    metadata_location: Vec<u8>,
    metadata_hash: Vec<u8>,
    ...
    TransactionPayload::EntryFunction(EntryFunction::new(
        ...
        ident_str!("create_single_step_or_multi_step_proposal").to_owned(),
        vec![],
        vec![
            bcs::to_bytes(&stake_pool).unwrap(),
            bcs::to_bytes(&execution_hash).unwrap(),
...
mod decoder {
    ...
    pub fn aptos_governance_create_single_step_or_multi_step_proposal(
        ...
        if let TransactionPayload::EntryFunction(script) = payload {
            Some(
                EntryFunctionCall::AptosGovernanceCreateSingleStepOrMultiStepProposal {
                    stake_pool: bcs::from_bytes(script.args().get(0)?).ok()?,
                    execution_hash: bcs::from_bytes(script.args().get(1)?).ok()?,
                    metadata_location: bcs::from_bytes(script.args().get(2)?).ok()?,
                    metadata_hash: bcs::from_bytes(script.args().get(3)?).ok()?,
...
static SCRIPT_FUNCTION_DECODER_MAP: once_cell::sync::Lazy<EntryFunctionDecoderMap> =
    once_cell::sync::Lazy::new(|| {
        ...
        map.insert(
            "aptos_governance_create_single_step_or_multi_step_proposal".to_string(),
            Box::new(decoder::aptos_governance_create_single_step_or_multi_step_proposal),
        );
        map.insert(
